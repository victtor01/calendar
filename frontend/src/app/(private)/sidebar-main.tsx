...
export function SidebarRelative() {
  ...
  return (
    <Sidebar
      ...
      className={`w-[4rem] lg:w-[15rem] text-gray-800 border-r dark:border-zinc-800 z-20 dark:text-white bg-white dark:bg-zinc-950   relative items-center lg:items-start flex flex-col font-semibold ${fontOpenSans}`}
    >
        ...
        <div className="flex font-normal flex-col gap-1  relative flex-nowrap w-full ">
          {pages.map(({ name, icon: Icon, href }, index: number) => {
            ...
            const selectedClass = selected
              ? "opacity-100 cursor-default text-white bg-gradient-45 from-zinc-800 to-zinc-950 dark:from-white dark:to-white dark:text-gray-700"
